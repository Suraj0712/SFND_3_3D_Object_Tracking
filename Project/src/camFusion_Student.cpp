...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    {
    	cv::KeyPoint prev_pnt = kptsPrev[kptMatch.queryIdx];
    	cv::KeyPoint curr_pnt = kptsCurr[kptMatch.trainIdx];
    	distances.push_back(sqrt(pow(prev_pnt.pt.x - curr_pnt.pt.x, 2) + pow(prev_pnt.pt.y - curr_pnt.pt.y, 2)));
    }

	double mean_dist = accumulate(distances.begin(), distances.end(), 0.0);
    mean_dist = mean_dist / distances.size();

    double stdev = 0.0;

    for (double distance : distances) 
    {
    	stdev += pow(distance - mean_dist, 2);
    }
...
{
    vector<double> prev_vector, curr_vector; //Need to create this as values are pass by reference and we concern only about x

    for(auto i: lidarPointsPrev)
    {
        prev_vector.push_back(i.x);
    }

    for(auto i: lidarPointsCurr)
    {
        curr_vector.push_back(i.x);
    }
    ...
    if(prev_vector.size() % 2 == 0)
    {
        d_prev = (prev_vector[prev_vector.size()/2] + prev_vector[(prev_vector.size()/2) -1]) / 2;
    }
    else
    {
        d_prev = prev_vector[prev_vector.size()/2];
    }
    
    if(curr_vector.size() % 2 == 0)
    {
        d_curr = (curr_vector[curr_vector.size()/2] + curr_vector[(curr_vector.size()/2) -1]) / 2;
    }
    else
    {
        d_curr = curr_vector[curr_vector.size()/2];
    }
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    int prev_curr_box_score[prev_box_size][curr_box_size] = {0};
    ...
    for (auto it = matches.begin(); it != matches.end() - 1; it++)
    {
        ...
        prev_box_id_list.clear();
        curr_box_id_list.clear();
